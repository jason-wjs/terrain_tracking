...
def test_pair_frame_sampler_default_adaptive_update_does_not_collapse_to_one_bin():
  sampler = PairFrameSampler(
  ...
  entropy, top1_prob = sampler.sampling_metrics()

  assert float(top1_prob) < 0.25
  assert float(entropy) > 0.9
