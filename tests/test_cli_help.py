...
def test_general_mid_blocks_exp_scripts_target_pair_dataset_training() -> None:
  ...
  assert (
    "TT-Tracking-TerrainOracleHeightLongScanPhpRewardGeneral-Unitree-G1"
    in height_longscan_play_script
  )
  assert "pair_dataset_mid_blocks.jsonl" in height_longscan_play_script
  assert (
    "mid_blocks_general_g1_oracle_height_longscan_phpreward_n16384_adaptive"
    in height_longscan_play_script
  )
  assert "tt_latest_checkpoint_file" in height_longscan_play_script
  assert "terrain_tracking.tasks.general_terrain_tracking.scripts.play" in (
    height_longscan_play_script
  )
...
def test_general_running_height_longscan_exp_script_targets_pair_dataset_training() -> None:
  ...
  assert (
    "running_general_g1_oracle_height_longscan_phpreward_full_n16384_adaptive"
    in train_script
  )
  assert "MAX_PAIRS=\"${MAX_PAIRS:-}\"" in train_script
  assert "NUM_ENVS=\"${NUM_ENVS:-16384}\"" in train_script
  assert "MAX_ITERATIONS=\"${MAX_ITERATIONS:-50000}\"" in train_script
  assert "tt_train_general_pair_dataset_exp" in train_script
